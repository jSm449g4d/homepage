    ...
    if request.method == "POST":
        ...
        if(_dataDict["order"]=="login"):
            ...
            with closing(sqlite3.connect(dbname)) as conn:
                ...
                if (_data["passhash"]==passhash):
                    token = jwt.encode({"id": _data["id"],"timestamp":_data["timestamp"]}, pyJWT_pass, algorithm="HS256")
                    return json.dumps({"message":"processed","user":_data["user"],"token":token}, ensure_ascii=False)
        ...
        if(_dataDict["order"]=="signin"):
            ...
            with closing(sqlite3.connect(dbname)) as conn:
                ...
                # create account
                cur.execute('INSERT INTO account(user,passhash,timestamp) values(?,?,?)',
        ...
        if(_dataDict["order"]=="account_delete"):
            ...
            with closing(sqlite3.connect(dbname)) as conn:
                conn.row_factory = sqlite3.Row 
                cur = conn.cursor()
                cur.execute("DELETE FROM account WHERE id = ? AND timestamp = ? ;",[token["id"],token["timestamp"]])
                conn.commit()
                return json.dumps({"message":"processed"}, ensure_ascii=False)
            return json.dumps({"message":"rejected"})
